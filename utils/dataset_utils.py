...
class Dataset(object):
    ...
    def sample(self, batch_size, utd=1):
        indx = np.random.randint(self.size, size=batch_size * utd)
...
class ReplayBuffer(Dataset):
    ...
    def online_initialize_with_dataset(self, num):
        indices = np.random.randint(self.size, size=num)

        self.observations[self.size:self.size+num] = self.observations[indices]
        self.actions[self.size:self.size+num] = self.actions[indices]
        self.rewards[self.size:self.size+num] = self.rewards[indices]
        self.masks[self.size:self.size+num] = self.masks[indices]
        self.dones_float[self.size:self.size+num] = self.dones_float[indices]
        self.next_observations[self.size:self.size+num] = self.next_observations[indices]

        self.insert_index += num
        self.size += num
